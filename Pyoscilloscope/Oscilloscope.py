...
class Interface:
    def __init__(self, ip_address, commands_file=None, verbose=None):
        ...
        try:
            ...
            print(
                "Error: A timeout has occurred trying to connect to the scope. Please try reconnecting."
            )
        ...
        if not os.path.isdir(commands_directory_path):
            print(
                "Error: The oscilloscope_commands directory:",
                commands_directory_path,
                "does not exist",
            )
            print(
                "Please check that the oscilloscope_commands directory is inside Pyoscilloscope directory"
            )
            return
        ...
        if commands_file == None:
            ...
            if not os.path.isfile(commands_file_path):
                ...
                print(
                    "Please check that default.json is inside the oscilloscope_commands directory"
                )
                return
            ...
            try:
                ...
                print(
                    "Error: A failure has occurred opening the default command file at:",
                    commands_file_path,
                    "or loading json format",
                )
                return
        else:
            ...
            if not os.path.isfile(commands_file_path):
                ...
                print(
                    "Please check that,",
                    commands_file,
                    "is inside the oscilloscope_commands directory",
                )
                return
        ...
        self.display = Display(
            self.__instr,
            self.__command_list["channel_expression"],
            self.__command_list["display"],
            self.__command_list["request_expression"],
        )

        self.attenuation = Attenuation(
            self.__instr,
            self.__command_list["channel_expression"],
            self.__command_list["attenuation"],
            self.__command_list["request_expression"],
        )
        ...
        self.voltage_range = Voltage_Range(
            self.__instr,
            self.__command_list["channel_expression"],
            self.__command_list["voltage_range"],
            self.__command_list["request_expression"],
        )

        self.voltage_division = Voltage_Division(
            self.__instr,
            self.__command_list["channel_expression"],
            self.__command_list["voltage_division"],
            self.__command_list["request_expression"],
        )
        ...
        self.voltage_offset = Voltage_Offset(
            self.__instr,
            self.__command_list["channel_expression"],
            self.__command_list["voltage_offset"],
            self.__command_list["request_expression"],
        )

        self.measure = Measure_Element(
            self.__instr,
            self.__command_list["channel_expression"],
            self.__command_list["voltage_offset"],
            self.__command_list["request_expression"],
        ...
        self.trigger_mode = Trigger_Type(
            self.__instr,
            self.__trigger_mode_commands,
            self.__command_list["channel_expression"],
            self.__command_list["trigger_mode"],
            self.__command_list["request_expression"],
        )

        self.wave_preamble = Wave_Preamble(
            self.__instr,
            self.__command_list["channel_expression"],
            self.__command_list["wave_preamble"],
            self.__command_list["request_expression"],
        )

        self.wave_form = Wave_Data(
            self.__instr,
            self.__command_list["channel_expression"],
            self.__command_list["wave_one_command"],
            self.__command_list["wave_data"],
            self.__command_list["request_expression"],
        )
    ...
    def time_range(self):
        ...
        if self.__command_list["time_range"] == self.__not_applicable_command:
            print(
                "Error: This scope does not feature time range according to the .json commands file. No changes made to scope"
            )
            return
        ...
        try:
            ...
            print(
                "Error: Invalid key: time_range or request_expression at time_range getter. Please check the .json file. No data received"
            )
            return
        except:
            print(
                ...
                request,
                "to the scope at time_range getter. Possible scope timeout. No data received",
            )
            return
        ...
        try:
            ...
            print(
                "Error: Invalid key: time_range at time_range getter. Please check the .json file. No data received"
            )
            return
        except:
            print(
                ...
                scope_value,
                "at time_range getter. Unable to parse and retrieve value or SI unit. No data received",
            )
            return
        ...
        if return_value != self._time_range:
            print(
                "Warning: current scope time range setting is",
                return_value,
                "rather than",
                self._time_range,
                " Setting to actual value",
                return_value,
            )
    ...
    def time_range(self, value):
        ...
        if self.__command_list["time_division"] == self.__not_applicable_command:
            print(
                "Error: This scope does not feature time division according to the .json commands file. No changes made to scope"
            )
            return
        ...
        if self._time_range != None:
            ...
            try:
                ...
                print(
                    "Error: Invalid key: time_range at time_range setter. Please check the .json file. No changes made to scope"
                )
            except:
                print(
                    "Error: A failure has occurred writing",
                    self.__command_list["time_range"],
                    "to the scope at time_range setter. Possible scope timeout",
                )
    ...
    def time_division(self):
        ...
        try:
            request = (
                self.__command_list["time_division"] + self.__command_list["request_expression"]
            )
            scope_value = self.__instr.ask(request)
        except KeyError:
            print(
                "Error: Invalid key: time_division or request_expression at time_division getter. Please check the .json file. No data received"
            )
            return
        except:
            print(
                "Error: A failure has occurred writing",
                request,
                "to the scope at time_division getter. Possible scope timeout. No data received",
            )
            return
        ...
        try:
            ...
            print(
                "Error: Invalid key: time_division at time_division getter. Please check the .json file. No data received"
            )
            return
        except:
            print(
                "Error: Unexpected return string from scope:",
                scope_value,
                "at time_division getter. Unable to parse and retrieve value or SI unit. No data received",
            )
            return
        ...
        if return_value != self._time_division:
            print(
                "Warning: current scope time division setting is",
                return_value,
                "rather than",
                self._time_division,
                " Setting to actual value",
    ...
    def time_division(self, value):
        ...
        if self._time_division != None:
            ...
            try:
                ...
                print(
                    "Error: Invalid key: time_division at time_division setter. Please check the .json file. No changes made to scope"
                )
            except:
                print(
                    "Error: A failure has occurred writing",
                    self.__command_list["time_division"],
                    "to the scope at time_division setter. Possible scope timeout",
                )
    ...
    def time_delay(self):
        ...
        if self.__command_list["time_delay"] == self.__not_applicable_command:
            print(
                "Error: This scope does not feature time delay according to the .json commands file. No changes made to scope"
            )
            return
        ...
        try:
            ...
            print(
                "Error: Invalid key: time_delay or request_expression at time_delay getter. Please check the .json file. No data received"
            )
            return
        except:
            print(
                "Error: A failure has occurred writing",
                request,
                "to the scope at time_delay getter. Possible scope timeout. No data received",
            )
            return
        ...
        try:
            ...
            print(
                "Error: Invalid key: time_delay at time_delay getter. Please check the .json file. No data received"
            )
            return
        except:
            print(
                "Error: Unexpected return string from scope:",
                scope_value,
                "at time_delay getter. Unable to parse and retrieve value or SI unit. No data received",
            )
            return
        ...
        if return_value != self._time_delay:
            print(
                "Warning: current scope time delay setting is",
                return_value,
                "rather than",
                self._time_delay,
                " Setting to actual value",
    ...
    def time_delay(self, value):
        ...
        if self._time_delay != None:
            try:
                ...
                print(
                    "Error: Invalid key: time_delay at time_delay setter. Please check the .json file. No changes made to scope"
                )
            except:
                print(
                    "Error: A failure has occurred writing",
                    self.__command_list["time_delay"],
                    "to the scope at time_delay setter. Possible scope timeout",
                )
    ...
    def trigger_sweep(self, value):
        ...
        if self._trigger_sweep != None:
            request = (
                self.__command_list["trigger_sweep"] + " " + self.__trigger_sweep_commands[value]
            )
            self.__instr.write(request)
    ...
    def acquisition_mode(self, value):
        ...
        if self.acquisition_mode != None:
            request = (
                self.__command_list["acquire_mode"] + " " + self.__acquisition_mode_commands[value]
            )
            self.__instr.write(request)
    ...
    def identify(self):
        ...
        if self.__command_list["identify"] == self.__not_applicable_command:
            print(
                "Error: This scope does not feature identify according to the .json commands file. No changes made to scope"
            )
            return
        # Write the command to the scope, if the key is not in the command list catch and print the error. If a timeout occurs or some other error catch and print a message
        try:
            ...
            print(
                "Error: Invalid key: identify at identify(). Please check the .json file. No changes made to scope"
            )
        except:
            print(
                "Error: A failure has occurred writing",
                self.__command_list["identify"],
                "to the scope at identify(). Possible scope timeout",
            )
    ...
    def clear(self):
        ...
        if self.__command_list["clear"] == self.__not_applicable_command:
            print(
                "Error: This scope does not feature clear according to the .json commands file. No changes made to scope"
            )
            return
        # Write the command to the scope, if the key is not in the command list catch and print the error. If a timeout occurs or some other error catch and print a message
        try:
            ...
            print(
                "Error: Invalid key: clear at clear(). Please check the .json file. No changes made to scope"
            )
        except:
            print(
                "Error: A failure has occurred writing",
                self.__command_list["clear"],
                "to the scope at clear(). Possible scope timeout",
            )
    ...
    def reset(self):
        ...
        if self.__command_list["reset"] == self.__not_applicable_command:
            print(
                "Error: This scope does not feature reset according to the .json commands file. No changes made to scope"
            )
            return
        # Write the command to the scope, if the key is not in the command list catch and print the error. If a timeout occurs or some other error catch and print a message
        try:
            ...
            print(
                "Error: Invalid key: reset at reset(). Please check the .json file. No changes made to scope"
            )
        except:
            print(
                "Error: A failure has occurred writing",
                self.__command_list["reset"],
                "to the scope at reset(). Possible scope timeout",
            )
    ...
    def auto_setup(self):
        ...
        if self.__command_list["auto_setup"] == self.__not_applicable_command:
            print(
                "Error: This scope does not feature auto_setup according to the .json commands file. No changes made to scope"
            )
            return
        # Write the command to the scope, if the key is not in the command list catch and print the error. If a timeout occurs or some other error catch and print a message
        try:
            ...
            print(
                "Error: Invalid key: auto_setup at auto_setup(). Please check the .json file. No changes made to scope"
            )
        except:
            print(
                "Error: A failure has occurred writing",
                self.__command_list["auto_setup"],
                "to the scope at auto_setup(). Possible scope timeout",
            )
    ...
    def force_trigger(self):
        ...
        if self.__command_list["force_trigger"] == self.__not_applicable_command:
            print(
                "Error: This scope does not feature force_trigger according to the .json commands file. No changes made to scope"
            )
            return
        # Write the command to the scope, if the key is not in the command list catch and print the error. If a timeout occurs or some other error catch and print a message
        try:
            ...
            print(
                "Error: Invalid key: force_trigger at force_trigger(). Please check the .json file. No changes made to scope"
            )
        except:
            print(
                "Error: A failure has occurred writing",
                self.__command_list["auto_setup"],
                "to the scope at force_trigger(). Possible scope timeout",
            )

    def display_measurement(self, channel, measurement_type):
        command_format = (
            self.__command_list["measure_display"]
            + " "
            + self.__measurement_commands[measurement_type]
            + ", "
            + self.__command_list["channel_expression"]
            + str(channel)
        )
    ...
    def write(self, expression):
        ...
        try:
            ...
            print(
                "Error: A failure has occurred writing",
                expression,
                "to the scope at write(). Possible scope timeout",
            )
    ...
    def ask(self, expression):
        ...
        try:
            ...
            print(
                "Error: A failure has occurred asking",
                expression,
                "to the scope at ask(). Possible scope timeout",
            )
